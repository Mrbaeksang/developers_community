...
1. **사용자에게 필요한 이미지 요청하기** (자동 검색 금지)
2. **사용자가 제공한 이미지 URL만 사용**
3. **이미지가 없으면 플레이스홀더 사용 후 사용자에게 추가 요청**

### 이미지 소스별 활용법:

**🔴 중요: 이미지 처리 우선순위**
1. **사용자 제공 이미지 URL** (최우선 사용)
2. **사용자에게 추가 이미지 요청** (부족한 경우)
3. **플레이스홀더 임시 사용** (Lorem Picsum) + 사용자에게 교체 요청
4. ❌ **절대 금지**: 자동 이미지 검색, MCP 도구 사용, LinkedIn, 개인 블로그
...
// ⚠️ 중요: 사용자에게 직접 요청, 자동 검색 절대 금지

// 2단계: 제공받은 이미지 URL 유효성 검증
// curl -I [이미지URL] 로 200 응답 확인

// 3단계: 사용자 제공 이미지로만 콘텐츠 작성
